// If defined shows the status of each task and the best schedule so far
// Comment out when using WriteLines() somewhere in the solving process
#define STATUS
...
namespace AfvalOphaler
{
    class Solver
    {
        ...
        /// </summary>
        /// <param name="threads">Amount of solvers to start</param>
        /// <param name="opCount">Amount of neighbors to generate per solver iteration</param>
        /// <param name="maxI">Maximum amount of iterations per solver</param>
        /// <param name="maxNoChange">Maximum amount of iterations without state change per solver</param>
        /// <returns>Solve as task with a scheduleresult that contains the best schedule as result</returns>
        public Task<ScheduleResult> StartSolving(int threads, int opCount, int maxI, int maxNoChange, int refreshTime) => Task.Run(() => Solve(threads, opCount, maxI, maxNoChange, refreshTime));
        /// <summary>
        ...
        private ScheduleResult Solve(int threads, int opCount, int maxI, int maxNoChange, int refreshTime)
        {
            ...
            Task statusUpdater = Task.Factory.StartNew(() => StatusUpdater(threads, refreshTime));
            // Start the solvers:
            for (int i = 0; i < threads; i++)
            {
                int index = i;
                tasks[index] = Task.Factory.StartNew(() => SolveOne(opCount, maxI, maxNoChange, index, ref bestResults[index]));
            }
        ...
        private void SolveOne(int opCount, int maxI, int maxNoChange, int taskID, ref ScheduleResult best)
        {
        ...
        private void StatusUpdater(int threads, int refreshTime)
        {
            Console.Clear();
            Stopwatch watch = Stopwatch.StartNew();
            int currBestIndex = -1;

            Timer updater = new Timer(refreshTime);
            ...
            void Updater_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
            {
                ...
                for (int i = 0; i < threads; i++)
                {
                    ...
                    #if STATUS
                    // Only get status if STATUS is defined:
                ...
                #if STATUS
                // Only print status if STATUS is defined:
